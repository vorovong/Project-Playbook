...
# 체크 2: mini-projects 변경 + progress 누락
MINI_CHANGED=$(git diff --name-only 2>/dev/null | grep -c 'mini-projects/' 2>/dev/null | tr -d ' \r\n' || echo 0)
PROGRESS_CHANGED=$(git diff --name-only 2>/dev/null | grep -c 'progress/' 2>/dev/null | tr -d ' \r\n' || echo 0)
# 빈 값이면 0으로
