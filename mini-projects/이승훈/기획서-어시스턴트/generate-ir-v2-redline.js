...
// 표에서 특정 행만 빨간색 (기존 행은 검정, 새로 추가된 행만 빨강)
function makeMixedTable(headers, blackRows, redRows) {
  const headerCells = headers.map(h => new TableCell({
    shading: { type: ShadingType.SOLID, color: "2C3E50" },
    children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: h, bold: true, size: 18, font: "맑은 고딕", color: "FFFFFF" })] })],
    borders: { top: thinBorder, bottom: thinBorder, left: thinBorder, right: thinBorder }
  ...
  const bRows = blackRows.map(row => new TableRow({
    children: row.map(cell => new TableCell({
      children: [new Paragraph({ children: [new TextRun({ text: cell, size: 18, font: "맑은 고딕" })] })],
      borders: { top: thinBorder, bottom: thinBorder, left: thinBorder, right: thinBorder }
    }))
  ...
  const rRows = redRows.map(row => new TableRow({
    children: row.map(cell => new TableCell({
      children: [new Paragraph({ children: [new TextRun({ text: cell, size: 18, font: "맑은 고딕", color: RED })] })],
      borders: { top: thinBorder, bottom: thinBorder, left: thinBorder, right: thinBorder }
    }))
  ...
  return new Table({
    width: { size: 100, type: WidthType.PERCENTAGE },
    rows: [new TableRow({ children: headerCells }), ...bRows, ...rRows]
  });
}
...
const doc = new Document({
  sections: [{
    ...
    children: [
      ...
      // 레드라인 안내
      empty(),
      new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "※ 빨간 글씨 = v1 대비 변경/추가된 내용", bold: true, size: 22, font: "맑은 고딕", color: RED })] }),
      ...
      // ★★★ [변경] Phase 1 목표 Q2→Q1 ★★★
      rH2("3-2. Phase 1 — 행궁동·수원 장악 (~27년 1분기)"),
      ...
      rH3("③ 완전 무료 전략"),
      rP("Phase 1~2 모두 구독료 0원. 경영자동화 전 기능을 무료로 개방한다."),
      rP("수익은 구매몰 수수료 3%와 데이터 판매로 만든다. 사장님에게 월정액을 받지 않는다."),
      empty(),
      ...
      empty(),
      rP("슈파 구매몰은 B2B 거래(사업자 간 식자재 구매)이므로 세금계산서 + 계좌이체 방식 적용."),
      rP("PG 결제 수수료 없이 수수료 3%가 온전히 슈파 마진이 된다."),
      rP("사장님도 세금계산서 발행으로 부가세 매입세액 공제를 받을 수 있어 양쪽 모두 유리."),
      empty(),
      // ★★★ [변경] KPI Q1 ★★★
      rPBold("KPI: 행궁동 100개소 → 수원 1,000개소"),
      rPBold("수익: 구매몰 수수료 3% + B2G 용역 (캐시카우). 구독료 없음"),

      pageBreak(),

      // ★★★ [변경] Phase 2 — 구독 섹션 삭제, 데이터+광고 중심 (전체 빨강) ★★★
      rH2("3-3. Phase 2 — 국내 확장 + 수익화 (27년~)"),
      rP("두 가지 수익 엔진이 동시에 켜지는 단계"),
      empty(),

      // ★★★ [변경] 데이터 판매를 첫 번째로 + 데이터 자산 강조 ★★★
      rH3("① 데이터 판매 — 핵심 수익 엔진"),
      rPBold("슈파만이 가진 데이터: 매입 + 매출 + 맥락의 통합 데이터"),
      rP("카드사는 매출 데이터만 있다. 유통사는 매입 데이터만 있다."),
      rP("슈파는 '어떤 가게가, 뭘 사서, 뭘 만들어, 얼마에 팔았는지'를 전부 안다."),
      rP("이 통합 데이터는 카드사도, 배민도, 캐시노트도 갖고 있지 않다."),
      empty(),

      // ★★★ [추가] 데이터 수집 구조 (전체 신규) ★★★
      rPBold("데이터 수집 구조 — 단계적 확장"),
      empty(),
      makeRedTable(["데이터 종류", "수집 방법", "수집 시점"], [
      ...
      rPBold("Phase 1 — 매입 데이터가 진짜 자산"),
      rP("카드 매출 데이터는 '얼마 팔았는지(총액)'만 안다. 어떤 메뉴가 몇 개 나갔는지는 모른다."),
      rP("하지만 슈파 구매몰의 매입 데이터는 '어떤 식자재를, 얼마나, 얼마에 사는지'를 정확히 안다."),
      rP("유통사·식자재 공급사에게는 카드 매출보다 매입 데이터가 훨씬 가치 있다."),
      rP("\"이 지역에서 양파가 월 몇 톤 소비되는가\" — 이걸 아는 곳은 슈파뿐이다."),
      empty(),
      rPBold("매입 + 매출 총액이 합쳐지면:"),
      rP("→ 원가율(매입÷매출) 자동 계산 = 금융사 신용평가에 핵심 지표"),
      rP("→ \"행궁동 한식집 평균 원가율 42%, 월매출 1,800만원, 2인 운영\""),
      rP("→ 이 수준의 통합 인사이트를 제공할 수 있는 플레이어는 슈파뿐이다."),
      empty(),

      rPBold("Phase 2 — 품목별 판매 데이터로 확장"),
      rP("클라우드 POS(토스POS, 페이히어 등)와 제휴하면 메뉴별 판매 수량까지 확보된다."),
      rP("배달앱(배민, 쿠팡이츠) 연동으로 배달 주문의 품목별 데이터도 수집 가능."),
      rP("마이데이터 사업자 등록을 통해 카드 매출 직접 연동으로 전환하여 비용을 절감한다."),
      empty(),

      rPBold("데이터 수집 로드맵"),
      makeRedTable(["단계", "수집하는 것", "알 수 있는 것"], [
      ...
      rPBold("데이터 판매 대상"),
      makeRedTable(["데이터 구매자", "활용", "예상 가격대"], [
      ...
      empty(),
      rP("첫 번째 데이터 바이어: 수원시 (기존 2년 협업 관계 → 상권 정책 데이터 리포트 납품 가능)"),
      rP("레퍼런스: 캐시노트(KCD)는 170만 사업장 기반 연결 매출 1,428억원. 사업장당 약 8만원/년 수준."),
      empty(),

      rH3("② 광고 수익"),
      rP("유통사·공급사 → 소상공인 대상 타겟 광고"),
      rP("슈파 구매몰 내 추천 상품, 배너, 프로모션 영역"),
      empty(),

      rH3("③ 상권 기금 선순환 (Ecosystem Fund)"),
      rQuote("\"내 거래가 우리 동네를 지킨다\""),
      rP("슈파는 단순 도구가 아니라 지역 생태계 플랫폼이다."),
      rP("구매몰 수수료와 데이터 판매 수익의 일부를 공존 기금으로 적립하여,"),
      rP("소상공인이 만든 가치가 소상공인에게 돌아가는 선순환을 만든다."),
      empty(),
      makeRedTable(["영역", "예시"], [
      ...
      empty(),
      rPBold("투자자 관점: 공존 기금이 만드는 '지역 귀속감'은 경쟁사가 복제할 수 없는 유저 잔존율의 핵심 동력이다. 비용이 아니라 해자(moat)."),
      ...
      h2("3개년 추정 손익 (P&L)"),
      p("(단위: 억원)"),
      makeRedTable(["구분", "2026년", "2027년", "2028년"], [
        ["매출", "2.1", "5.7", "26.9"],
      ...
      empty(),
      rBullet("2026년 (기반 구축): B2G 용역으로 버티면서 유저 확보에 올인. 적자 9,000만원은 투자금으로 커버"),
      rBullet("2027년 (전환점): 구매몰 수수료 + 데이터 판매 + 광고가 동시에 켜짐. 흑자 전환"),
      rBullet("2028년 (폭발적 성장): 전국 5,000개소로 확장. 구매몰 거래액이 레버리지 → 영업이익 15.9억"),
      empty(),
      ...
      empty(),
      // ★★★ [변경] LOI 설명 (신규) ★★★
      rPBold("구매의향서(LOI) = MVP 전 수요 검증"),
      rP("시스템이 완성되기 전에, 행궁동 소상공인 대상으로 품목별 구매의향서를 확보한다."),
      rP("\"이 품목을, 이 가격이면 공동구매하겠다\"는 서면 확인 = 투자자에게 수요가 이미 존재한다는 증거."),
      rP("동시에 어떤 품목에 수요가 집중되는지 데이터를 확보 → MVP 우선순위 결정에 활용."),
      empty(),

      // ★★★ [변경] Q2→Q1 ★★★
      rH2("Phase 1-2: 수원 1,000개소 (~27년 1분기)"),
      makeTable(["채널", "전략", "목표"], [
      ...
      h2("MOAT (경쟁 우위)"),
      // ★★★ [변경] 데이터 해자 추가 — 기존 행 + 신규 행 ★★★
      makeMixedTable(["우위", "설명"],
        // 기존 행 (검정)
        [
...
// 저장
const outputPath = "C:/Users/leeha/OneDrive/문서/카카오톡 받은 파일/IR-기획서-v2-레드라인.docx";
Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync(outputPath, buffer);
  console.log("✅ 저장 완료:", outputPath);
  console.log("");
  console.log("=== 변경 내용 요약 (빨간 글씨) ===");
  console.log("1. Phase 1 목표: Q2 → Q1(1분기)로 변경");
  console.log("2. 프리미엄 기능 → '고급 기능'으로 표현 변경 (구독 없이)");
  console.log("3. 완전 무료 전략: 구독 전환 관련 내용 삭제, B2B 거래 설명 추가");
  console.log("4. Phase 2 전면 개편: 구독 삭제, 데이터 판매를 핵심 수익으로");
  console.log("5. [신규] 데이터 수집 구조 — 매입/매출/맥락 단계별 확장");
  console.log("6. [신규] 상권 기금 선순환 (Ecosystem Fund)");
  console.log("7. Business Model: 구독 라인 삭제");
  console.log("8. P&L: 구독 매출 라인 삭제, 숫자 재조정");
  console.log("9. Go-to-Market: LOI(구매의향서) 추가");
  console.log("10. Phase 1-2: Q2 → Q1로 변경");
  console.log("11. MOAT: '통합 데이터' 해자 추가");
  console.log("12. Traction: LOI 항목 추가");
  console.log("13. 마일스톤: Q2 → Q1로 변경");
}).catch(err => {
