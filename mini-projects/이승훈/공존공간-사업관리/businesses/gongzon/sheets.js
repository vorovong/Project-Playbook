...
function build월결산(config, month, data) {
  ...
  blocks.push(table([
    ...
    row(['*임대료 매출 제외', '', '', '']),
    row(['부가세(10%)', fmt(부가세), '전기세 (미식가 차감)', fmt(utilities.전기세)]),
    row(['카드수수료(1.3%)', fmt(카드수수료_전체), '가스비 (미식가 차감)', fmt(utilities.가스비)]),
    row(['기타지출', fmt(기타지출), '수도세 (미식가 차감)', fmt(utilities.수도세)]),
    // 2026-04-07 추가 — 공존 부담 전기 (1층1호, 1층2호, 2층1호, 2층2호, 3층). 현재 합계 미반영.
    row(['', '', '⚠️ 공존부담 전기(미반영)', fmt(utilities.전기_공존부담_고지서 || 0)]),
